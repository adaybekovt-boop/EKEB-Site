...
const responsiveUrl = (source: string, width: number, format?: "avif" | "webp") => {
  if (!source.startsWith("http")) return source;
  try {
    const url = new URL(source);
    url.searchParams.set("w", String(width));
    url.searchParams.set("q", width <= 960 ? "68" : "72");
    ...
    return (
      ...
      >
            ...
            <div className="fx-fixed" ref={fixedRef}>
              <div className="fx-bgs" aria-hidden="true">
                {sections.map((s, i) => {
                  ...
                  const source = nearby ? responsiveUrl(s.background, 1600) : TRANSPARENT_PIXEL;
                  const sourceSet = nearby
                    ? [640, 960, 1280, 1600]
                        .map((width) => `${responsiveUrl(s.background, width, "webp")} ${width}w`)
                        .join(", ")
                    : undefined;
                  return (
                      ...
                      {s.renderBackground ? (
                        ...
                        <>
                          <img
                            ...
                            className={`fx-bg-img ${nearby ? "is-nearby" : ""}`}
                          />
        ...
        <style jsx>{`
          ...
          .fx-fixed-section {
            height: ${Math.max(1, total + 1) * 55}vh;
            position: relative;
          }
          .fx-fixed {
            position: sticky;
